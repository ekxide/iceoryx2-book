...
1. Enable cross-host communication with the `iox2 gateway` CLI, without
   touching application code
2. Embed the gateway in your own application for full control over its
   execution
3. Drive the gateway in polling and reactive modes
...
```{tip}
A gateway can be implemented for any communication mechanism, not just network
communication. All that is required is an implementation of the `Backend`
traits available in [`iceoryx2-gateway-backend`](
https://docs.rs/iceoryx2-gateway-backend/latest/iceoryx2_gateway_backend/).

For example, the gateway has been used to extend communication between
...
gateway can be configured to work with any approach. See [execution control
patterns](/tutorials/execution-control-patterns) for an overview.

```{note}
The gateway currently only supports the publish-subscribe and event messaging
...
With the applications set up, we now need to spin up the gateway to extend
the communication over the network. The gateway can either be run in a separate
process or embedded into the application. First, let's use the `iceoryx2-cli`
to run the gateway in a separate process.
...
Next, the CLI for a specific gateway backend must be installed. For this
article, we use the Zenoh backend:

```sh
cargo install iceoryx2-integrations-zenoh-gateway-cli
```

The command `iox2 gateway --list` can be used to verify that the installation
was successful. This command lists all installed gateway backends discovered
by the CLI.

The gateway should be installed on both hosts.

### Running a gateway process

The CLI can now be used to start a gateway process on both hosts:

```console
iox2 gateway zenoh
```

Each gateway discovers `iceoryx2` services, both on its own host and over
the communication mechanism used by the chosen backend implementation.
Larry's gateway sees `larry/battery` and `larry/position` and forwards all
samples published to them over the wire. The dashboard gateway receives these
...
By default the gateway polls for new services and pending samples every 100 ms,
however since the event-based communication pattern is used, it is possible
to configure the gateway to execute reactively.

The gateway running on Larry only has work to do when new samples are published
on the services that need to be propagated:

```console
iox2 gateway zenoh --listener "larry/battery" --listener "larry/position"
```

The gateway running on the dashboard host only has work to do when data arrives
over the backend:

```console
iox2 gateway zenoh --reactive-backend
```

Nothing here is application code. Both applications were written, compiled,
and deployed with no knowledge that a gateway exists. Communicating across the
hosts is purely a deployment decision.

### Embedding the gateway

```{note}
The embeddable gateway is currently only available as a Rust library. C++, C,
and Python applications must use the gateway from a separate process with the
`iox2 gateway` CLI shown above; the remainder of this section applies to Rust.
```

Running the gateway as its own process is convenient, but it's not the only
option. The gateway is a library, and it spawns no threads of its own so you
...
First add the generic gateway and your chosen backend as dependencies:

```toml
iceoryx2-gateway = { version = "X.Y.Z" }
iceoryx2-integrations-zenoh-gateway-backend = { version = "X.Y.Z" }
```

The gateway can then be instantiated in your application via the provided
...
dropped in, even your own. Specifying `polled()` in the build returns a gateway
...
versa. Calling the two on the polling gateway in a timed loop
replicates the default behavior of the CLI.

Now let's take a look at how to replicate the reactive configurations provided
by the gateway. Recapping the details, both gateways must be set up to react
to different things:

1. Larry's gateway reacts to the services notified when a sample is published
1. The dashboard's gateway reacts to data arriving over the network

Since Larry's gateway does not react to network communication, it can be
...
The dashboard gateway only cares about data arriving over the network, so it is
created in reactive mode, which provides a listener along with the created
gateway. This listener is notified when new data is available from the backend
...
only in what decides when those calls happen. Once again, the gateway has no
...
gateway. In our examples above, we have been using Zenoh, so the gateways were
created with:

```rust
Gateway::<ipc::Service, ZenohBackend<ipc::Service>>
```

The first parameter is the local `iceoryx2` service type; the second is the
backend. The gateway itself knows nothing about Zenoh. It is written against
the backend traits, and `discover()` and `propagate()` are the same calls
regardless of what carries the bytes.

Switching over to a different mechanism is therefore only a change of the type
specified on gateway creation:

```rust
Gateway::<ipc::Service, SomeOtherBackend<ipc::Service>>
```
...
to implement your own using the traits provided in [`iceoryx2-gateway-backend`](
https://docs.rs/iceoryx2-gateway-backend/latest/iceoryx2_gateway_backend/).
...
The control-flow pattern the dashboard and gateway employ.
```
